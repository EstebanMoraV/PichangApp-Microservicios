...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `aldefy-compose-skill-jetpack-compose-expert-skill`                                |
| Version    | 1.0.2                                     |
| Author     | aldefy                                 |
...
**GitHub:** [aldefy/compose-skill](https://github.com/aldefy/compose-skill) — ⭐ 80 | Forks: 5
...
A Jetpack Compose expert skill that provides actionable guidance for building modern Android UIs. It advises on state management (use of @Composable, remember, mutableStateOf, derivedStateOf, and state hoisting), composable structure and view composition patterns, Modifier chains and layout modifiers, LazyColumn/LazyRow performance, navigation (NavHost, passing and scoping state), animations, side-effects (LaunchedEffect, DisposableEffect), theming (MaterialTheme, Material3), accessibility best practices, and performance optimization driven by androidx source analysis. Use this skill when designing or debugging Compose screens, optimizing recomposition, deciding where to hoist state, implementing navigation or animations, or improving accessibility and rendering performance. Core advantages include concrete API recommendations, common anti-pattern detection, citation of framework behavior from source, and pragmatic refactors to make Compose code predictable, testable, and performant.
...
```bash
npx -y @lobehub/market-cli skills install aldefy-compose-skill-jetpack-compose-expert-skill
```
...
npx -y @lobehub/market-cli skills install aldefy-compose-skill-jetpack-compose-expert-skill --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install aldefy-compose-skill-jetpack-compose-expert-skill --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install aldefy-compose-skill-jetpack-compose-expert-skill --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install aldefy-compose-skill-jetpack-compose-expert-skill --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install aldefy-compose-skill-jetpack-compose-expert-skill --version 1.0.2
```
...
```bash
npx -y @lobehub/market-cli skills rate aldefy-compose-skill-jetpack-compose-expert-skill --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment aldefy-compose-skill-jetpack-compose-expert-skill -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment aldefy-compose-skill-jetpack-compose-expert-skill
```
...
```bash
npx -y @lobehub/market-cli skills comments aldefy-compose-skill-jetpack-compose-expert-skill
```
