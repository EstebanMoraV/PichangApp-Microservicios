...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `dpconde-claude-android-skill`                                |
| Version    | 1.0.1                                     |
| Author     | dpconde                                 |
...
**GitHub:** [dpconde/claude-android-skill](https://github.com/dpconde/claude-android-skill) — ⭐ 68 | Forks: 13
...
This Skill scaffolds production-quality Android applications following Google’s official architecture guidance and NowInAndroid best practices. Use it to generate full Kotlin projects, Jetpack Compose screens, ViewModels, repositories, Room schemas and migrations, Hilt dependency injection setup, navigation and multi-module feature boundaries. It produces MVVM-aligned code templates, Gradle/Kotlin DSL configurations, state management patterns, and testable units (unit and instrumentation test suggestions). Typical use cases include bootstrapping new apps, adding features or modules, refactoring toward modular architecture, and answering architecture-pattern questions. Core advantages are consistent, maintainable, and testable code that reduces boilerplate, accelerates development, and aligns with Google recommendations. The Skill also offers guidance on CI/CD, performance, offline support, and trade-offs for scalable, production-ready Android apps.
...
```bash
npx -y @lobehub/market-cli skills install dpconde-claude-android-skill
```
...
npx -y @lobehub/market-cli skills install dpconde-claude-android-skill --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install dpconde-claude-android-skill --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install dpconde-claude-android-skill --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install dpconde-claude-android-skill --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install dpconde-claude-android-skill --version 1.0.1
```
...
```bash
npx -y @lobehub/market-cli skills rate dpconde-claude-android-skill --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment dpconde-claude-android-skill -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment dpconde-claude-android-skill
```
...
```bash
npx -y @lobehub/market-cli skills comments dpconde-claude-android-skill
```
